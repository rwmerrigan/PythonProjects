...
def list_events(
    ...
    with sqlite3.connect(DB_PATH) as cn:
        ...
        for event in events:
            ...
            days_remaining = (event_date - datetime.date.today()).days
            if days_remaining < 0:
                msg = f"[red]{abs(days_remaining)} days ago[/red]"
            ...
            table.add_row(
                ...
                event.event_date,
                str(event.priority),
                "Yes" if bool(event.is_private) else "No",
                msg,
            )
